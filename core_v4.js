...
(function(){
  ...
  const makeRng = (seedStr) => {
    ...
    if (!s) return {rand: Math.random, shuffle: (arr)=>{for(let i=arr.length-1;i>0;i--){const j=Math.floor(Math.random()*(i+1));[arr[i],arr[j]]=[arr[j],arr[i]];} return arr;}};
    // xmur3 + sfc32
  ...
  function buildUI(root){
    ...
    const controls = el("div",{class:"eks-card"},
      ...
      el("div",{class:"eks-row",style:"margin-top:8px"},
        ...
        fieldSelect("Shuffle list","shuffleList",["on","off"],"on"),
        fieldSelect("Use matrix","useMatrix",["off","on"],"off"),
        el("div",{class:"eks-field"}, 
          el("label",{class:"eks-label"},"Matrix MK values (for MK vary chamber)"),
          el("input",{class:"eks-in mono", id:"mkVals", placeholder:"Example: 0,2,4,6,8 or 0-9 or 0-9:2"})
        ),
        el("div",{class:"eks-field"}, 
          el("label",{class:"eks-label"},"Matrix CK values A (for CK vary A)"),
          el("input",{class:"eks-in mono", id:"ckValsA", placeholder:"Example: 0,2,4,6,8 or 0-9 or 0-9:2"})
        ),
        el("div",{class:"eks-field"}, 
          el("label",{class:"eks-label"},"Matrix CK values B (for CK vary B)"),
          el("input",{class:"eks-in mono", id:"ckValsB", placeholder:"Example: 1,3,5,7,9 or 0-9"})
        ),
        el("div",{class:"eks-field sm"},
  ...
  function onGenerate(){
    ...
    const shuffleListOn = $("#shuffleList").value === "on";
    const useMatrix = $("#useMatrix") && $("#useMatrix").value === "on";
    const mvList = useMatrix ? parseVals($("#mkVals").value) : null;
    const aList = useMatrix ? parseVals($("#ckValsA").value) : null;
    const bList = useMatrix ? parseVals($("#ckValsB").value) : null;
    ...
    for (const mv of mkVaryCandidates){
      for (const [aIdx,bIdx] of ckPairs){
        for (const mode of modes){
          const attempt = tryBuild({tmk, ds, macs, step, parityOn, randomizeOn, shuffleListOn, rng, mv, aIdx, bIdx, mkCount, ckPerMk, doorCount, Avals:(useMatrix ? aList : stepAlign(mode.A)), Bvals:(useMatrix ? bList : stepAlign(mode.B)), MVvals:(useMatrix ? mvList : null)});
  ...
  function tryBuild({tmk, ds, macs, step, parityOn, randomizeOn, shuffleListOn, rng, mv, aIdx, bIdx, mkCount, ckPerMk, doorCount, Avals, Bvals, MVvals}){
    ...
    if (shuffleListOn){
      // Shuffle within each MK group so the list doesn't look like counting
      const groups = new Map();
      cks.forEach((ck,i)=>{
        if (!groups.has(ck.mkSym)) groups.set(ck.mkSym, []);
        groups.get(ck.mkSym).push(i);
      });
      // Build new order of indices
      let newIdx = [];
      for (const [mkSym, idxs] of groups.entries()){
        if (randomizeOn) rng.shuffle(idxs);
        newIdx.push(...idxs);
      }
      // Shuffle MK group order too
      if (randomizeOn) rng.shuffle(newIdx);

      const cks2 = newIdx.map(i=>cks[i]);
      const doors2 = newIdx.map(i=>doors[i]);
      const pin2 = newIdx.map(i=>pinning[i]);

      // Renumber doors sequentially to match shuffled order
      for (let i=0;i<cks2.length;i++){
        const doorNum = i+1;
        const sym = `C${String(doorNum).padStart("3","0")}`;
        cks2[i].doorNum = doorNum; cks2[i].sym = sym;
        doors2[i].doorNum = doorNum; doors2[i].ckSym = sym;
        pin2[i].doorNum = doorNum; pin2[i].ckSym = sym;
      }
      cks.length = 0; doors.length = 0; pinning.length = 0;
      cks.push(...cks2); doors.push(...doors2); pinning.push(...pin2);
    }
